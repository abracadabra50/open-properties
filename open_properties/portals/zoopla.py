...
class ZooplaAdapter(PortalAdapter):
    ...
    def build_search_url(self, config: SearchConfig) -> str:
        ...
        min_price = f"&price_min={config.min_price}" if config.min_price else ""
        max_price = f"&price_max={config.max_price}" if config.max_price else ""
        path = "to-rent" if config.transaction == "rent" else "for-sale"
        return f"https://www.zoopla.co.uk/{path}/property/{location}/?beds_min={config.min_beds}{min_price}{max_price}&results_sort=newest_listings"
    ...
    def parse_properties(self, markdown: str, fetch_url: str, transaction: str = "sale") -> List[Dict[str, Any]]:
        properties = []
    ...
    def search(self, config: SearchConfig) -> Dict[str, Any]:
        ...
        if not markdown:
            return {"portal": self.name, "provider": self.name, "country": "GB", "fetched_at": utc_now_iso(), "count": 0, "fetch_urls": [url], "properties": [], "error": "Firecrawl CLI not available or API key not set"}
        properties = self.parse_properties(markdown, url, config.transaction)
        return {"portal": self.name, "provider": self.name, "country": "GB", "fetched_at": utc_now_iso(), "count": len(properties), "fetch_urls": [url], "properties": properties, "note": "Uses Firecrawl CLI/API"}
