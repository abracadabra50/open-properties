...
class RightmoveAdapter(PortalAdapter):
    ...
    def build_search_url(self, config: SearchConfig, index: int = 0, location_id: str = "") -> str:
        ...
        min_price = f"&minPrice={config.min_price}" if config.min_price else ""
        ...
        path = "property-to-rent" if config.transaction == "rent" else "property-for-sale"
        return (
            f"https://www.rightmove.co.uk/{path}/find.html"
            f"?locationIdentifier={loc}&minBedrooms={config.min_beds}{min_price}{max_price}{property_types}&sortType=6&index={index}"
        )
    ...
    def parse_property(self, prop: Dict[str, Any], fetch_url: str, location_id: str, transaction: str = "sale") -> Optional[Dict[str, Any]]:
        ...
        price_period = "week" if " pw" in price_lower or "per week" in price_lower else ("month" if "pcm" in price_lower or "per month" in price_lower else "")
        ...
        pc_match = re.search(r"\b([A-Z]{1,2}\d[A-Z\d]?\s*\d[A-Z]{2})\b", address, re.I)
        ...
        return normalise_listing({
            ...
            "price_period": price_period if transaction == "rent" else "",
    ...
    def search(self, config: SearchConfig) -> Dict[str, Any]:
        ...
        for location_id in location_ids:
            for page_num in range(config.max_pages):
                ...
                for raw in raw_properties:
                    parsed = self.parse_property(raw, url, location_id, config.transaction)
        ...
        return {"portal": self.name, "provider": self.name, "country": "GB", "fetched_at": utc_now_iso(), "count": len(properties), "fetch_urls": fetch_urls, "properties": properties}
